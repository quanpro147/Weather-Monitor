...
- Missing value imputation (forward-fill → interpolation → column median)
...
# Columns expected in the weather_daily table (excluding city_id / date PKs)
...
# Hard physical bounds — values outside these are impossible and should be nulled
PHYSICAL_BOUNDS: dict[str, tuple[float, float]] = {
    "temperature_2m_max": (-20.0, 55.0),
    "temperature_2m_min": (-20.0, 55.0),
    "temperature_2m_mean": (-20.0, 55.0),
    "rain_sum": (0.0, 1500.0),
    "shortwave_radiation": (0.0, 35.0),
...
def cap_outliers_iqr(df: pd.DataFrame, multiplier: float = 2.5) -> tuple[pd.DataFrame, int]:
    ...
    for col in WEATHER_NUMERIC_COLS:
        ...
        # Apply physical bounds as additional guard
...
def impute_missing(df: pd.DataFrame) -> tuple[pd.DataFrame, int]:
    ...
    """
    before_nulls = int(df[WEATHER_NUMERIC_COLS].isnull().sum().sum() if WEATHER_NUMERIC_COLS else 0)
    ...
    # Final fallback: column-wide median for any remaining NaNs
...
def quality_report(df: pd.DataFrame) -> dict[str, Any]:
    ...
    return {
        ...
        "per_column_missing": {
            col: int(df[col].isnull().sum()) for col in existing_cols
        },
    }
