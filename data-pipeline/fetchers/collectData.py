...
def main():
    ...
    # Đọc danh sách thành phố từ file city_id.csv
    ...
    print(f"Starting BULK data collection from {start_date} to {end_date}...")

    # Cập nhật: Chỉ fetch cho những thành phố CHƯA CÓ đủ data
    for _, row in cities_df.iterrows():
        city_id = row['CityId']
        lat = row['Latitude']
        lon = row['Longitude']

        # Kiểm tra xem thành phố này đã được fetch data bulk (từ đầu 2017) chưa
        cursor.execute("SELECT MIN(date) FROM public.weather_daily WHERE city_id = %s", (city_id,))
        result = cursor.fetchone()[0]
        # Nếu đã fetch rồi (có tồn tại ngày 2017-01-01) thì bỏ qua để tiết kiệm limit
        if result and result <= datetime.date(2017, 1, 5): 
            continue

        print(f"Fetching historical data for {row['City']} (ID: {city_id})...")
        ...
        try:
            ...
            error_msg = str(e)
            print(f"-> Error fetching {row['City']}: {error_msg}")
            if 'Hourly API request limit exceeded' in error_msg:
                print(">>> Hourly limit reached. Pausing for 60 minutes...")
                time.sleep(3600) # Nghỉ 1 tiếng
                # Bạn sẽ cần tự viết retry logic ở đây nếu muốn không bị bỏ sót thành phố này do bỏ qua vòng lặp,
                # Tuy nhiên vì có tính năng Resume, chạy lại file script sau 60 phút sẽ tự động bỏ qua các thành phố đã cào.
        
        time.sleep(2.5) # Limit requests (tránh minutely limit)
